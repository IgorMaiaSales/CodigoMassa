...
import { Github } from 'lucide-react'; // Certifique-se de ter instalado: npm install lucide-react

export default function LoginPage() {
  const router = useRouter();

  // 1. Monitora o usuário globalmente (para redirecionar assim que logar)
  const [user] = useAuthState(auth);

  // 2. Hooks de Login Social
  ...
  useEffect(() => {
    if (user) {
      router.push('/problems'); // Ou '/dashboard'
    }
  }, [user, router]);

  // Captura erros de login social para exibir na tela
  const socialError = errorGoogle || errorGithub;

  return (
    <div className="min-h-screen flex selection:bg-brand-green selection:text-white font-sans text-brand-text bg-brand-dark">
      
      {/* Lado Esquerdo: Branding (Mantido idêntico ao seu) */}
      ...
      <div className="w-full lg:w-1/2 bg-[#13201E] flex items-center justify-center p-6 relative overflow-y-auto">
        ...
        <div className="w-full max-w-md space-y-8 mt-12 lg:mt-0">
          ...
          {/* Exibição de Erros Sociais (Se houver) */}
          {socialError && (
             <div className="p-3 text-sm text-brand-error bg-brand-error/10 border border-brand-error rounded-md text-center">
                {socialError.message.includes('popup-closed') 
                    ? 'Login cancelado.' 
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <button 
                ...
                className="w-full flex items-center justify-center gap-2 bg-brand-surface border border-brand-border hover:border-brand-muted/50 hover:bg-[#1e3430] text-brand-text font-bold py-3 px-4 rounded-lg transition-all duration-200 group disabled:opacity-50"
            >
          ...
          {/* O FormComponent cuida do login e senha */}
          <FormComponent />

          <div className="text-center mt-6">
            <p className="text-sm text-brand-muted">
              Não tem uma conta?{' '}
              <Link href="/auth/register" className="font-bold text-brand-greenLight hover:text-brand-yellow hover:underline transition-colors">Criar conta grátis</Link>
            </p>
          </div>
