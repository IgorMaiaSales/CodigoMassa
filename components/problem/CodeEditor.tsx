...
import { ChevronDown, RotateCcw, Send, Terminal, AlertCircle, CheckCircle, AlertTriangle, XCircle, Check } from 'lucide-react';
...
export default function CodeEditor({ problemSlug }: CodeEditorProps) {
  ...
  const [code, setCode] = useState(selectedLang.placeholder);
  ...
  const renderResult = (data: any) => {
    ...
    setConsoleOutput(
      <div className="animate-fade-in p-2 space-y-3">
        ...
        {/* Lista de Subtarefas (Detalhe do que passou/falhou) */}
                        ...
                        <div key={sub.id} className={`flex items-center justify-between p-2 rounded text-xs border ${
                            ...
                            <div className="flex items-center gap-2">
                                {sub.passed ? (
                                    ...
                                    <XCircle size={14} className="text-brand-error opacity-60" />
                                )}
                                <span className={sub.passed ? "text-brand-text" : "text-brand-muted"}>
                                    Subtarefa {sub.id}
                                </span>
                            </div>
                            <span className={`font-mono font-bold ${
                                sub.passed ? "text-brand-green-light" : "text-brand-muted"
                            }`}>
        ...
        {data.stderr && (
            <div className="bg-black/40 p-3 rounded border border-brand-border overflow-x-auto">
                ...
                <pre className="text-brand-error text-[10px] font-mono whitespace-pre-wrap">
                    {data.stderr}
                </pre>
            </div>
        )}
    ...
    <div className="flex-1 flex flex-col bg-brand-panel relative min-w-0 h-full border-l border-brand-border">
      ...
      <div className="h-12 bg-brand-dark border-b border-brand-border flex items-center justify-between px-4 shrink-0">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => setCode(selectedLang.placeholder)} className="p-1.5 rounded-lg hover:bg-brand-border text-brand-muted hover:text-white transition" title="Resetar Código">
            <RotateCcw size={16} />
          </button>
          <button onClick={handleSubmit} disabled={isRunning} className="px-4 py-1.5 rounded-lg bg-brand-green hover:bg-brand-green/80 text-white border border-brand-green/50 text-xs font-bold transition flex items-center gap-2 group disabled:opacity-50 shadow-md shadow-brand-green/10">
      ...
      <div className="flex-1 relative flex flex-col min-h-0 bg-brand-dark">
        <div className="flex-1 flex overflow-hidden">
          ...
          <textarea ref={textareaRef} value={code} onChange={(e) => setCode(e.target.value)} onKeyDown={handleKeyDown} onScroll={handleScroll} className="flex-1 bg-transparent text-brand-text caret-brand-green-light outline-none resize-none font-mono text-sm leading-6 p-4 whitespace-pre border-none focus:ring-0" spellCheck="false" autoComplete="off" />
        </div>
      </div>
      {/* CONSOLE */}
